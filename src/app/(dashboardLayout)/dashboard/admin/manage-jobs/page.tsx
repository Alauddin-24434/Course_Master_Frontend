...
export default function ManageJobsPage() {
   ...
   const { data: appsData, isLoading: isLoadingApps } = useGetAllApplicationsQuery(
      { page: appPage, limit: 10 },
      { skip: !isAppsModalOpen }
   )
   ...
   const jobs = (jobsData as any)?.data?.data?.jobs || (jobsData as any)?.data?.jobs || []
   const applications = (appsData as any)?.data?.data?.applications || (appsData as any)?.data?.applications || []
   ...
   return (
      <div className="p-8 space-y-10">
         ...
         <div className="bg-card border border-border rounded-[2rem] overflow-hidden shadow-sm">
            <div className="p-6 border-b border-border bg-secondary/10 flex items-center justify-between">
               <div className="relative w-full max-w-md">
                  ...
                  <input
                     ...
                     className="w-full h-11 pl-11 pr-4 bg-background border border-border rounded-xl text-xs font-medium focus:ring-2 focus:ring-primary/20 outline-none transition-all"
                  />
               </div>
            ...
            {/* Pagination Controls */}
            {((jobsData as any)?.data?.data?.totalPages || (jobsData as any)?.data?.totalPages || 0) > 1 && (
               <div className="border-t border-border/50 bg-muted/5">
                  <Pagination
                     currentPage={jobPage}
                     totalPages={(jobsData as any)?.data?.data?.totalPages || (jobsData as any)?.data?.totalPages || 0}
                     onPageChange={(newPage) => setJobPage(newPage)}
                  />
               </div>
            )}
               ...
               <div className="relative w-full max-w-5xl bg-card border border-border rounded-[3rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
                  <div className="p-10 space-y-8">
                     ...
                     <div className="max-h-[60vh] overflow-y-auto pr-4 custom-scrollbar">
                        ...
                        {((appsData as any)?.data?.data?.totalPages || (appsData as any)?.data?.totalPages || 0) > 1 && (
                            <div className="mt-8 pt-8 border-t border-border/50">
                                <Pagination 
                                    currentPage={appPage}
                                    totalPages={(appsData as any)?.data?.data?.totalPages || (appsData as any)?.data?.totalPages || 0}
